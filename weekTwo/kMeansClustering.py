...
#TODO: Try out a different way to assign membership: check a point's distance to cluster center
#inital assignment of cluster
...
n = 500
K = 5
# DEFAULT - UNIFORM distribution RANDOM NUMBERS
...
c = np.tile(dataRange,(K,1)).T*(np.random.random((2, K))/3)+np.tile(dataMean,(K,1)).T
...
while not isCompleted:
    ...
    for i, members in enumerate(clusterMembers):
        if members is not None:
            ...
            # TODO: Need to assing a new position if the cluster center has no member
    ...
    for i in range(0, K):
        ax.scatter(c[0, i], c[1, i], c=randomColors(i), marker="^")

    # for j in range(0, n):
    #     ax.scatter(X[0,j], X[1,j], c=randomColors(ownship[j]))
